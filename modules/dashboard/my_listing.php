...
// Restore Logic for Data Fetching
$user_id = $user['id'];
$pdo = db();
    ...
    <div class="mb-8">
      <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
        ...
        <div class="flex flex-col sm:flex-row gap-3">
          ...
          echo getExportButton('my_listings'); ?>
          <a href="<?= url('index.php?p=sellingOption') ?>"
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-sm">
            <thead class="bg-gradient-to-r from-blue-50 to-purple-50 text-gray-700 uppercase tracking-wide">
              <tr>
                <th class="text-left py-3 px-3 font-semibold whitespace-nowrap">Listing</th>
                <th class="text-left py-3 px-3 font-semibold whitespace-nowrap">Status</th>
                <th class="text-left py-3 px-3 font-semibold whitespace-nowrap">Price</th>
                <th class="text-left py-3 px-3 font-semibold whitespace-nowrap">Info</th>
                <th class="text-left py-3 px-3 font-semibold whitespace-nowrap">Revenue</th>
                <th class="text-left py-3 px-3 font-semibold whitespace-nowrap">Proofs</th>
                <th class="text-left py-3 px-3 font-semibold whitespace-nowrap">Actions</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-100">
              <?php foreach ($listings as $listing): ?>
                <tr class="hover:bg-blue-50/50 transition-colors" data-listing-id="<?= $listing['id'] ?>">
                  ...
                  <td class="py-4 px-4">
                    <div class="flex items-center gap-3">
                      <div class="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0">
                        <i class="fas fa-<?= ($listing['type'] === 'youtube') ? 'play' : 'globe' ?> text-blue-600"></i>
                      </div>
                      <div class="min-w-0">
                        <h3 class="font-semibold text-gray-900 text-sm"><?= htmlspecialchars($listing['name'] ?: 'Untitled Listing') ?></h3>
                        <p class="text-xs text-gray-500 capitalize"><?= htmlspecialchars($listing['type']) ?></p>
                        <p class="text-xs text-gray-400">
                          <i class="fas fa-calendar mr-1"></i>
                          <?= date('M j, Y', strtotime($listing['created_at'])) ?>
                        </p>
                        <?php if ($listing['proof_count'] > 0): ?>
                  ...
                  <!-- Proofs Column -->
                  <td class="py-3 px-3">
                    <?php if (!empty($listing['proof_images'])): ?>
                      <div class="flex items-center gap-2">
                        <?php
                        $proofPaths = explode(',', $listing['proof_images']);
                        $displayCount = min(3, count($proofPaths));
                        for ($i = 0; $i < $displayCount; $i++):
                          ...
                          // Path cleanup and URL generation
                          $url = url(ltrim($proofPath, '/'));
                        ?>
                          <div class="w-8 h-8 rounded-full border-2 border-white overflow-hidden cursor-pointer"
...
<?php
// Fix for potential double "public/" in paths
?>

<script>
  // Define BASE constant globally if not already defined
  if (typeof BASE === 'undefined') {
    window.BASE = "<?php echo BASE; ?>";
  }

  // Define API_BASE_PATH for polling.js
  // Ensure it points to the /api directory, handling cases where BASE includes /public/
  const baseStr = "<?php echo BASE; ?>";
  const apiBase = baseStr.replace('public/', '');
  window.API_BASE_PATH = apiBase.replace(/\/+$/, '') + '/api';

  console.log('🔧 BASE constant defined:', window.BASE || BASE);
  console.log('🔧 API_BASE_PATH defined:', window.API_BASE_PATH);
    ...
    startPolling({
      listings: (newListings) => {
        ...
        if (newListings.length > 0) {
          console.log('🔍 User IDs in received listings:', newListings.map(l => ({
            id: l.id,
            user_id: l.user_id,
            name: l.name
          })));
  ...
  function addListingToTable(listing) {
    ...
    const askingPrice = parseFloat(listing.asking_price || 0).toLocaleString('en-US', {
      minimumFractionDigits: 0,
      maximumFractionDigits: 0
    });
    const monthlyRevenue = parseFloat(listing.monthly_revenue || 0).toLocaleString('en-US', {
      minimumFractionDigits: 0,
      maximumFractionDigits: 0
    });

    // Helper to generate proof thumbnails HTML safely
    const generateProofsHtml = () => {
      if (!listing.proof_images || listing.proof_count == 0) return '<span class="text-gray-400">-</span>';
      ...
      for (let i = 0; i < displayCount; i++) {
        ...
        // Fix path using window.BASE
        const cleanPath = proofPath.startsWith('/') ? proofPath.substring(1) : proofPath;
        const fullUrl = window.BASE + cleanPath;
      ...
      return `
        <div class="flex items-center gap-2">
            <div class="flex -space-x-1">${thumbnailsHtml}</div>
            <span class="text-xs text-gray-500 block mt-1">
                ${listing.proof_count} file${listing.proof_count > 1 ? 's' : ''}
            </span>
        </div>`;
    ...
    row.innerHTML = `
      ...
      <!-- Price Column -->
      <td class="py-3 px-3">
        <div class="font-semibold text-gray-900 text-sm">$${askingPrice}</div>
      </td>
      ...
      <!-- Revenue Column -->
      <td class="py-3 px-3">
        <div class="font-medium text-green-600 text-sm">$${monthlyRevenue}/mo</div>
      </td>
      ...
      <td class="py-3 px-3">
        <div class="flex items-center gap-2">
          <a href="${window.BASE}listing/${listing.id}"
             class="text-blue-600 hover:text-blue-800 transition-colors text-sm" title="View">
            <i class="fas fa-eye"></i>
          </a>
          ${listingType === 'youtube' ? `
            <a href="${window.BASE}updateYtListing/${listing.id}" 
               class="text-green-600 hover:text-green-800 transition-colors text-sm" title="Edit">
              <i class="fas fa-edit"></i>
            </a>
          ` : `
            <a href="${window.BASE}updateWebListing/${listing.id}" 
               class="text-green-600 hover:text-green-800 transition-colors text-sm" title="Edit">
              <i class="fas fa-edit"></i>
            </a>
          `}
    ...
    if (emptyStateMessage && tableStructure && emptyStateMessage.style.display !== 'none') {
  ...
  Are you sure you want to delete "${listingName}" ? `, {
      ...
      if (result === true) {
        ...
        setTimeout(function() {
          window.location.href = `${window.BASE}dashboard/delete_listing?id=${listingId}`;
        }, 500);
      } else {
