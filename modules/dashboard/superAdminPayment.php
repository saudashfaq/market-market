...
<head>
    ...
    <title>Payments & Finance Dashboard</title>
...
<body class="bg-gray-50">
            ...
            <div class="stat-card bg-white rounded-xl shadow-sm p-4 md:p-6 border border-gray-100">
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <h2 class="text-2xl md:text-3xl font-bold text-gray-900 mt-1">$<?= number_format($totalEscrow, 2) ?></h2>
            ...
            <div class="stat-card bg-white rounded-xl shadow-sm p-4 md:p-6 border border-gray-100">
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <h2 class="text-2xl md:text-3xl font-bold text-gray-900 mt-1">$<?= number_format($totalCommission, 2) ?></h2>
            ...
            <div class="stat-card bg-white rounded-xl shadow-sm p-4 md:p-6 border border-gray-100">
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <h2 class="text-2xl md:text-3xl font-bold text-gray-900 mt-1">$<?= number_format($monthlyRevenue, 2) ?></h2>
                ...
                <?php else: ?>
                    ...
                    <?php if ($hasPaymentData && $paymentsPagination): ?>
                        <div class="flex flex-col md:flex-row justify-between items-center mt-6 pt-4 border-t border-gray-200">
                            ...
                            <?php 
                            $extraParams = ['p' => 'dashboard', 'page' => 'superAdminPayment', 'tab' => 'payments'];
                            if ($search) $extraParams['search'] = $search;
                            if ($status) $extraParams['status'] = $status;
                            echo renderPagination($paymentsPagination, url('index.php'), $extraParams, 'pg'); 
                            ?>
                        </div>
                    <?php endif; ?>
                <?php endif; ?>
            ...
            <div id="transactions" class="tab-content <?= $activeTab !== 'transactions' ? 'hidden' : '' ?>">
                ...
                <?php if ($hasTransactionData && $transactionsPagination): ?>
                    <div class="flex flex-col md:flex-row justify-between items-center mt-6 pt-4 border-t border-gray-200">
                        ...
                        <?php 
                        $extraParams = ['p' => 'dashboard', 'page' => 'superAdminPayment', 'tab' => 'transactions'];
                        if ($search) $extraParams['search'] = $search;
                        if ($status) $extraParams['status'] = $status;
                        echo renderPagination($transactionsPagination, url('index.php'), $extraParams, 'pg'); 
                        ?>
                    </div>
                <?php endif; ?>
            <?php endif; ?>
    ...
    <script>
        ...
        if (typeof startPolling !== 'undefined') {
            ...
            startPolling({
                orders: (newOrders) => {
                    console.log('💳 New orders/payments detected:', newOrders.length);
                    if (newOrders.length > 0) {
                        // Update payment stats without reload
                        updatePaymentStats(newOrders);
                        
                        // Add to transactions table
                        addNewTransactions(newOrders);
                        
                        // Show notification
                        if (typeof PollingUIHelpers !== 'undefined') {
                            PollingUIHelpers.showBriefNotification(
                                `${newOrders.length} new payment(s) detected!`,
                                'success'
                            );
                        }
                    }
            ...
            function updatePaymentStats(newOrders) {
                // Update total transactions count
                const totalEl = document.querySelector('[data-stat="total-transactions"]');
                if (totalEl) {
                    const current = parseInt(totalEl.textContent.replace(/,/g, '')) || 0;
                    totalEl.textContent = (current + newOrders.length).toLocaleString();
                ...
                if (completedCount > 0) {
                    const completedEl = document.querySelector('[data-stat="completed-transactions"]');
                    if (completedEl) {
                        const current = parseInt(completedEl.textContent.replace(/,/g, '')) || 0;
                        completedEl.textContent = (current + completedCount).toLocaleString();
                        completedEl.classList.add('animate-pulse');
                        setTimeout(() => completedEl.classList.remove('animate-pulse'), 1000);
                    }
                }
                ...
                if (totalRevenue > 0) {
                    ...
                    if (revenueEl) {
                        const current = parseFloat(revenueEl.dataset.value || 0);
                        const newTotal = current + totalRevenue;
                        revenueEl.dataset.value = newTotal;
                        revenueEl.textContent = '$' + newTotal.toLocaleString();
            ...
            // Helper function to add new transactions to table
            function addNewTransactions(newOrders) {
                ...
                newOrders.slice(0, 5).forEach(order => {
                    ...
                    row.innerHTML = `
                        <td class="py-4 px-6 font-medium">TXN${order.id || 'NEW'}</td>
                        <td class="py-4 px-6">${order.buyer_name || 'N/A'}</td>
                        <td class="py-4 px-6">${order.seller_name || 'N/A'}</td>
                        <td class="py-4 px-6 font-semibold">$${parseFloat(order.amount || 0).toLocaleString()}</td>
                        <td class="py-4 px-6">
