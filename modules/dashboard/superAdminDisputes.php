...
<div class="max-w-7xl mx-auto p-4 md:p-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
      <form method="GET" class="flex flex-wrap gap-4 items-end">
        ...
        <div class="flex gap-2">
          ...
          <a href="<?= url('dashboard/superAdminDisputes') ?>" class="px-6 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition-colors flex items-center">
  ...
  if ($hasRealData && !empty($disputeRecords)) {
    $openCount = count(array_filter($disputeRecords, function ($item) {
      return $item['status'] === 'open';
    }));
    $investigatingCount = count(array_filter($disputeRecords, function ($item) {
      return $item['status'] === 'investigating';
    }));
    $resolvedCount = count(array_filter($disputeRecords, function ($item) {
    ...
    <div class="bg-white rounded-xl shadow-sm p-4 md:p-6 border border-gray-100">
      <div class="flex justify-between items-start">
        <div>
          ...
          <h2 id="open-disputes-count" class="text-2xl md:text-3xl font-bold text-red-500 mt-1"><?= $openCount ?></h2>
          <?php if ($openCount > 0): ?>
    ...
    <div class="bg-white rounded-xl shadow-sm p-4 md:p-6 border border-gray-100">
      <div class="flex justify-between items-start">
        <div>
          ...
          <h2 id="investigating-count" class="text-2xl md:text-3xl font-bold text-yellow-500 mt-1"><?= $investigatingCount ?></h2>
    ...
    <div class="bg-white rounded-xl shadow-sm p-4 md:p-6 border border-gray-100">
      <div class="flex justify-between items-start">
        <div>
          ...
          <h2 id="resolved-count" class="text-2xl md:text-3xl font-bold text-green-500 mt-1"><?= $resolvedCount ?></h2>
    ...
    <div class="bg-white rounded-xl shadow-sm p-4 md:p-6 border border-gray-100">
      <div class="flex justify-between items-start">
        <div>
          ...
          <h2 id="total-dispute-value" class="text-2xl md:text-3xl font-bold text-blue-500 mt-1">
            <?php if ($totalValue > 0): ?>
  ...
  <?php if ($hasRealData): ?>
    <div class="bg-white rounded-xl shadow-sm p-4 md:p-6 border border-gray-100">
                ...
                <td colspan="9" class="px-6 py-16 text-center">
                  <div class="flex flex-col items-center justify-center">
                    ...
                    <?php else: ?>
                      ...
                      <a href="<?= url('dashboard/superAdminDisputes') ?>" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm font-medium">
      ...
      <div class="mt-6 pt-4 border-t border-gray-200">
        ...
        echo renderPagination($pagination, url('dashboard/superAdminDisputes'), $extraParams, 'pg');
        ?>
      </div>
    </div>
  <?php endif; ?>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    if (typeof startPolling === 'function') {
      startPolling({
        disputes: function(newDisputes) {
          if (newDisputes && newDisputes.length > 0) {
            PollingUIHelpers.showBriefNotification(`${newDisputes.length} new disputes`);

            // Update stats
            newDisputes.forEach(d => {
              if (d.status === 'open') PollingUIHelpers.updateStatsCounter('open-disputes-count', 1);
              if (d.status === 'investigating') PollingUIHelpers.updateStatsCounter('investigating-count', 1);
              if (d.status === 'resolved') PollingUIHelpers.updateStatsCounter('resolved-count', 1);

              // Update total value (rough approximation parsing K or plain)
              const valEl = document.getElementById('total-dispute-value');
              if (valEl) {
                // This is tricky because the PHP formats as K (e.g. $1.5K). 
                // We might just want to reload the page or fetch fresh stats?
                // For now, let's just highlight it to show change
                valEl.classList.add('text-blue-600');
                setTimeout(() => valEl.classList.remove('text-blue-600'), 2000);
              }
            });
            ...
            if (tbody) {
              newDisputes.forEach(d => {
                if (!document.querySelector(`tr[data-dispute-id="${d.id}"]`)) {
                  ...
                  tr.className = 'hover-row transition-colors bg-red-50';
                  ...
                  } else if (d.status === 'investigating') {
                  ...
                  const priorityColor = (d.priority === 'high' ? 'text-red-500' : (d.priority === 'medium' ? 'text-yellow-500' : 'text-green-500'));

                  tr.innerHTML = `
                                    <td class="py-4 px-4">
                                        <div class="font-medium text-gray-800">${d.case_id || 'N/A'}</div>
                                        <div class="text-xs text-gray-500 mt-1">Opened: ${new Date(d.created_at).toLocaleDateString()}</div>
                                    </td>
                                    <td class="py-4 px-4">
                                        <div class="font-medium text-gray-800">ORD-${String(d.id).padStart(3, '0')}</div>
                                        <div class="text-xs text-gray-500 mt-1">${d.listing || 'Unknown'}</div>
                                    </td>
                                    <td class="py-4 px-4">
                                        <div class="font-medium text-gray-800">${d.buyer || 'Unknown'}</div>
                                        <div class="text-xs text-gray-500 mt-1">Buyer</div>
                                    </td>
                                    <td class="py-4 px-4">
                                        <div class="font-medium text-gray-800">${d.seller || 'Unknown'}</div>
                                        <div class="text-xs text-gray-500 mt-1">Seller</div>
                                    </td>
                                    <td class="py-4 px-4">
                                        <div class="font-medium text-gray-800">${d.reason || 'N/A'}</div>
                                        <div class="text-xs text-gray-500 mt-1">Dispute reason</div>
                                    </td>
                                    <td class="py-4 px-4">
                                        <div class="font-semibold text-gray-800">$${parseFloat(d.amount).toFixed(2)}</div>
                                        <div class="text-xs text-gray-500 mt-1">In dispute</div>
                                    </td>
                                    <td class="py-4 px-4">
                                        <div class="${priorityColor} font-medium">0 days</div>
                                        <div class="text-xs text-gray-500 mt-1">${d.priority} priority</div>
                                    </td>
                                    <td class="py-4 px-4">
                                        <span class="inline-flex items-center px-3 py-1 rounded-full ${statusClass} text-xs font-medium">
                                          <i class="fa ${statusIcon} mr-1.5"></i> ${d.status.charAt(0).toUpperCase() + d.status.slice(1)}
                                        </span>
                                    </td>
                                    <td class="py-4 px-4">
                                        <div class="flex flex-col gap-2">
                                          <button onclick="viewDispute(${d.id})" class="text-blue-600 hover:text-blue-800 text-sm font-medium flex items-center">
                                            <i class="fa fa-eye mr-1.5"></i> View Details
                                          </button>
                                          ${(d.status === 'open' || d.status === 'investigating') ? `
                                            <button onclick="resolveDispute(${d.id})" class="text-green-600 hover:text-green-800 text-sm font-medium flex items-center">
                                              <i class="fa fa-check mr-1.5"></i> Resolve
                                            </button>
                                          ` : ''}
                                          <button onclick="updatePriority(${d.id}, '${d.priority}')" class="text-orange-600 hover:text-orange-800 text-sm font-medium flex items-center">
                                            <i class="fa fa-exclamation-triangle mr-1.5"></i> Priority
                                          </button>
                                        </div>
                                    </td>
