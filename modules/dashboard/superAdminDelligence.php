...
<div class="max-w-7xl mx-auto p-4 md:p-6">
  ...
  <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
    ...
    <div class="flex items-center gap-4">
      ...
      echo getExportButton('due_diligence'); ?>
    </div>
  </div>
  ...
  if ($hasRealData && !empty($diligenceRecords)) {
    $pendingCount = count(array_filter($diligenceRecords, function ($item) {
      return $item['status'] === 'pending';
    }));
    $completedCount = count(array_filter($diligenceRecords, function ($item) {
      return $item['status'] === 'completed';
    }));
    $flaggedCount = count(array_filter($diligenceRecords, function ($item) {
          ...
          <tbody class="divide-y divide-gray-100 text-sm">
            ...
            <?php else: ?>
              <?php foreach ($diligenceData as $item): ?>
                ...
                $progressColor = $item['verification_score'] >= 80 ? 'bg-green-500' : ($item['verification_score'] >= 60 ? 'bg-yellow-500' : 'bg-red-500');
                ?>
