...
try {
  ...
  if ($activeSection === 'logs') {
    $whereClause = "WHERE l.user_id IS NOT NULL AND u.role = 'user'";
    $params = [];
    ...
    // Get listing changes with pagination - from users only
    $whereClause = "WHERE l.user_id IS NOT NULL AND u.role = 'user' AND (l.action LIKE '%listing%' OR l.action LIKE '%Listing%' OR l.details LIKE '%listing%')";
    $params = [];
    ...
    // Get payment logs with pagination - from users only
    $whereClause = "WHERE l.user_id IS NOT NULL AND u.role = 'user' AND (l.action LIKE '%payment%' OR l.action LIKE '%Payment%' OR l.details LIKE '%payment%' OR l.details LIKE '%escrow%')";
    $params = [];
  ...
  $listingChangesCountStmt = $pdo->prepare("
        ...
        FROM logs l 
        LEFT JOIN users u ON l.user_id = u.id
        WHERE l.user_id IS NOT NULL AND u.role = 'user' AND (l.action LIKE '%listing%' OR l.action LIKE '%Listing%' OR l.details LIKE '%listing%')
    ");
  ...
  $paymentLogsCountStmt = $pdo->prepare("
        ...
        FROM logs l 
        LEFT JOIN users u ON l.user_id = u.id
        WHERE l.user_id IS NOT NULL AND u.role = 'user' AND (l.action LIKE '%payment%' OR l.action LIKE '%Payment%' OR l.details LIKE '%payment%' OR l.details LIKE '%escrow%')
    ");
  ...
  <div class="max-w-7xl mx-auto p-4 md:p-6">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
      ...
      <div class="flex items-center gap-4">
        ...
        echo getExportButton('admin_reports'); ?>
      </div>
    </div>
