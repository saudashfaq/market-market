...
<script>
  ...
  function handleRejectOffer(offerId, listingName, amount) {
    console.log('🎯 handleRejectOffer called:', {
      offerId,
      listingName,
      amount
    });
...
<script>
  ...
  function confirmAcceptOffer(event, offerId, listingName, amount, totalOffers) {
    console.log('🎯 confirmAcceptOffer called with:', {
      event,
      offerId,
      listingName,
      amount,
    ...
    console.log('Accept offer confirmation triggered:', {
      offerId,
      listingName,
      amount,
      totalOffers
  ...
  function confirmRejectOffer(event, offerId, listingName, amount) {
    console.log('🎯 confirmRejectOffer called with:', {
      event,
      offerId,
      listingName,
      amount
    ...
    console.log('Reject offer confirmation triggered:', {
      offerId,
      listingName,
      amount
    });
...
<script>
  ...
  function showOfferDetails(offer) {
    ...
    document.getElementById('detailAskingPrice').textContent = '$' + parseFloat(offer.listing_price || 0).toLocaleString('en-US', {
    ...
    document.getElementById('detailOfferAmount').textContent = '$' + parseFloat(offer.amount).toLocaleString('en-US', {
      minimumFractionDigits: 2
    });
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Polling is now handled by polling-init.js
    // Legacy inline polling removed to prevent double-polling and errors
...
<script>
  ...
  function handleAcceptOfferDirect(offerId, listingName, amount, totalOffers) {
    console.log('🎯 SILENT Accept - No UI feedback:', {
      offerId,
      listingName,
      amount,
      totalOffers
  ...
  function handleRejectOfferDirect(offerId, listingName, amount) {
    console.log('🎯 SILENT Reject - No UI feedback:', {
      offerId,
      listingName,
      amount
    });
