...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['action'] === 'add') {
  ...
  if (!in_array($_POST['role'] ?? '', $validRoles)) {
    $validator->custom('role', function () {
      return false;
  ...
  if (!in_array($_POST['status'] ?? '', $validStatuses)) {
    $validator->custom('status', function () {
      return false;
...
<body class="bg-gray-50 min-h-screen text-gray-800">
  ...
  <script>
    ...
    async function deleteUser(id) {
      const confirmed = await showConfirm('Are you sure you want to delete this member? This action cannot be undone.', {
        title: 'Delete Member',
      ...
      try {
        ...
        showError('An error occurred while deleting the member.');
      }
    }
