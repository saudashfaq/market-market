  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="flex justify-between items-center py-6">
      ...
      <div class="flex items-center space-x-4">
        ...
        echo getExportButton('listing_verification'); ?>
      </div>
    </div>
  </div>
</div>
...
<div class="min-h-screen bg-gray-50">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6 mb-8">
      <?php
      // Get global stats for all listings regardless of pagination/filters
      $statsSql = "
          SELECT 
              COUNT(*) as total,
      ...
      $verified_count = $stats['approved'] ?? 0;
      ...
      <div class="bg-white rounded-xl shadow-sm p-4 md:p-6 border border-gray-100">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p id="stats-total" class="text-2xl md:text-3xl font-bold text-gray-900 mt-1"><?= $total_count ?></p>
          </div>
      ...
      <div class="bg-white rounded-xl shadow-sm p-4 md:p-6 border border-gray-100">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p id="stats-pending" class="text-2xl md:text-3xl font-bold text-yellow-500 mt-1"><?= $pending_count ?></p>
          </div>
      ...
      <div class="bg-white rounded-xl shadow-sm p-4 md:p-6 border border-gray-100">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p id="stats-verified" class="text-2xl md:text-3xl font-bold text-green-500 mt-1"><?= $verified_count ?></p>
          </div>
      ...
      <div class="bg-white rounded-xl shadow-sm p-4 md:p-6 border border-gray-100">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p id="stats-rejected" class="text-2xl md:text-3xl font-bold text-red-500 mt-1"><?= $rejected_count ?></p>
          </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-4 md:p-6 border border-gray-100">
        ...
        <div class="mt-4 md:mt-0 w-full md:w-auto">
          <form method="GET" class="flex flex-col md:flex-row gap-2 md:gap-2">
            ...
            <div class="flex gap-2">
              ...
              <a href="<?= url('dashboard/listingverification') ?>" class="flex-1 md:flex-initial px-4 py-2.5 md:py-2 bg-gray-500 text-white rounded-lg text-sm font-medium hover:bg-gray-600 transition-colors text-center">
      ...
      <?php if (empty($listings)): ?>
        ...
        <div class="text-center py-16">
          ...
          <?php if ($search || $status): ?>
            ...
            <a href="<?= url('dashboard/listingverification') ?>"
            ...
            <div class="<?= $cardClass ?> rounded-lg shadow-sm border border-gray-200 p-4">
              ...
              <div class="flex items-start gap-3 mb-3">
                <?php if ($l['proof_image']): ?>
                  <img src="/marketplace/public/<?= ltrim(htmlspecialchars($l['proof_image']), '/') ?>" alt="Proof" class="w-16 h-16 rounded-lg object-cover border shadow-sm flex-shrink-0">
                <?php else: ?>
              ...
              <div class="flex gap-2 pt-3 border-t border-gray-200">
                <a href="<?= url('listingDetail?id=' . $l['id']) ?>" class="flex-1 px-3 py-2 bg-blue-50 text-blue-600 rounded-lg text-center text-sm font-medium hover:bg-blue-100 transition-colors">
                ...
                <tr class="<?= $rowClass ?> transition-colors">
                  <td class="py-4 px-4">
                    <div class="flex items-center gap-3">
                      <?php if ($l['proof_image']): ?>
                        <img src="/marketplace/public/<?= ltrim(htmlspecialchars($l['proof_image']), '/') ?>" alt="Proof" class="w-12 h-12 rounded-lg object-cover border shadow-sm">
                      <?php else: ?>
                  ...
                  <td class="py-4 px-4">
                    <div class="flex items-center space-x-3">
                      <a href="<?= url('listingDetail?id=' . $l['id']) ?>" class="text-blue-600 hover:text-blue-800 p-2 rounded-lg hover:bg-blue-50 transition-colors" title="View Details">
      ...
      <div class="mt-6 pt-4 border-t border-gray-200">
        ...
        echo renderPagination($pagination, url('dashboard/listingverification'), $extraParams, 'pg');
        ?>
  ...
  <script>
      ...
      if (window.startPolling) {
        startPolling({
          listings: function(newListings) {
            ...
            if (pendingCount > 0) {
              // Update counters
              PollingUIHelpers.updateStatsCounter('#stats-total', pendingCount);
              PollingUIHelpers.updateStatsCounter('#stats-pending', pendingCount);

              PollingUIHelpers.showBriefNotification(`${pendingCount} new listings received`, 'success');

              // Add to table if on the first page or viewing all
              const tbody = document.querySelector('table tbody');
              if (tbody) {
                newListings.forEach(item => {
                  const exists = document.querySelector(`tr[data-record-id="${item.id}"]`) ||
                    Array.from(document.querySelectorAll('a[href*="id=' + item.id + '"]')).length > 0;

                  if (!exists && item.status === 'pending') {
                    PollingUIHelpers.addRecordToTable(item, 'table', (l) => {
                      return `
                        <td class="py-4 px-4">
                          <div class="flex items-center gap-3">
                             ${l.proof_image ? 
                               `<img src="/marketplace/public/${l.proof_image.replace(/^\//, '')}" alt="Proof" class="w-12 h-12 rounded-lg object-cover border shadow-sm">` : 
                               `<div class="w-12 h-12 bg-gray-100 rounded-lg flex items-center justify-center text-gray-400 border"><i class="fa-regular fa-image text-lg"></i></div>`
                             }
                            <div>
                              <div class="font-medium text-gray-800">${l.name || 'Untitled'}</div>
                              <div class="text-xs text-gray-500 mt-1 flex items-center">
                                <i class="fa fa-tag mr-1"></i> ${l.category || 'General'}
                              </div>
                            </div>
                          </div>
                        </td>
                        <td class="py-4 px-4">
                          <div class="font-medium">${l.seller_name || 'Unknown'}</div>
                          <div class="text-xs text-gray-500 mt-1 flex items-center">
                            <i class="fa fa-envelope mr-1"></i> ${l.email || 'N/A'}
                          </div>
                        </td>
                        <td class="py-4 px-4">
                          <a href="https://${l.url}" target="_blank" class="text-blue-600 hover:text-blue-800 hover:underline flex items-center">
                            <i class="fa fa-external-link-alt mr-2 text-sm"></i> ${l.url}
                          </a>
                        </td>
                        <td class="py-4 px-4">
                          <div class="font-semibold text-gray-800">$${(parseFloat(l.asking_price)||0).toLocaleString()}</div>
                          <div class="text-xs text-gray-500 mt-1 flex items-center">
                             <i class="fa fa-chart-line mr-1"></i> $${(parseFloat(l.monthly_revenue)||0).toLocaleString()}/mo
                          </div>
                        </td>
                        <td class="py-4 px-4">
                          <span class="inline-flex items-center px-3 py-1 rounded-full bg-yellow-100 text-yellow-700 text-xs font-medium">
                        ...
                        <td class="py-4 px-4">
                          <div class="flex items-center space-x-3">
                            <a href="${window.BASE}listingDetail?id=${l.id}" class="text-blue-600 hover:text-blue-800 p-2 rounded-lg hover:bg-blue-50 transition-colors" title="View Details">
                              <i class="fa-regular fa-eye"></i>
                            </a>
                            <button onclick="verifyListing(${l.id}, 'verify', this)" class="text-green-600 hover:text-green-800 p-2 rounded-lg hover:bg-green-50 transition-colors" title="Approve">
                              <i class="fa-regular fa-circle-check"></i>
    ...
    function verifyListing(listingId, action, buttonElement) {
      ...
      fetch(`${window.BASE}dashboard/admin_listing_verification?action=${action}&id=${listingId}&ajax=1`, {
          method: 'GET',
          headers: {
      ...
      `;
        ...
        .catch(error => {
          ...
          console.error('Request URL:', `${window.BASE}dashboard/admin_listing_verification?action=${action}&id=${listingId}&ajax=1`);
