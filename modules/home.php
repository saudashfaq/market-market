  ...
  <div class="container mx-auto px-6 relative z-10">
    ...
    <div class="text-center mb-14 animate-fade-in">
      <h2 class="text-4xl sm:text-5xl font-extrabold text-gray-900 mb-4">
        ...
        <span class="inline-block align-middle ml-2 px-3 py-1 bg-green-100 text-green-600 text-lg font-bold rounded-full shadow-sm transform rotate-3 border border-green-200">
          It's Free
        </span>
      </h2>
...
">
              ...
              <?php if ($user_logged_in): ?>
                <?php if ($listing['user_id'] != $user_id): ?>
                  <div class="flex gap-2">
                    ...
                    <button type="button"
                      onclick="window.location.href='<?= url('index.php?p=dashboard&page=conversation_create&listing_id=' . $listing['id'] . '&seller_id=' . $listing['user_id']) ?>'"
                      ...
                      <i class="fa-solid fa-comment-dots mr-2"></i> Buy Now
                    </button>
                    ...
                    <button type="button"
                      ...
                      <i class="fa-solid fa-comment-dots mr-2"></i> Make Offer
                    </button>
                  </div>
                <?php else: ?>
    ...
    <?php if ($user_logged_in): ?>
      ...
      <div class="mt-14 text-center animate-fade-in">
        <a href="<?= url('index.php?p=listing') ?>" class="group border border-blue-600 text-blue-600 hover:bg-blue-600 hover:text-white font-semibold px-8 py-3 rounded-md shadow-sm transition">
          View All Listings
    ...
    <div class="grid lg:grid-cols-3 gap-8 items-stretch">
      ...
      <div class="lg:col-span-2 bg-white/10 backdrop-blur-xl rounded-2xl p-8 flex flex-col justify-between shadow-xl hover:-translate-y-1 transition-transform duration-500">
        ...
        <div class="flex justify-end">
          <?php if ($user_logged_in): ?>
            <a href="<?= url('index.php?p=sellingOption') ?>" class="bg-white text-indigo-700 font-semibold px-8 py-2.5 rounded-lg shadow-md hover:bg-gray-100 transition duration-300">
              Start Selling
            </a>
          <?php else: ?>
        ...
        ${userLoggedIn ? `
          ${!isOwnListing ? `
            <div class="flex gap-2">
              <!-- Buy Now Button -->
              <button type="button" 
                      onclick="window.location.href = window.BASE + 'index.php?p=dashboard&page=conversation_create&listing_id=' + ${listing.id} + '&seller_id=' + ${listing.user_id}"
                      data-listing-id="${listing.id}"
          ...
          <!-- View Details Button -->
          <div class="mt-2">
            <a href="${window.BASE}index.php?p=listingDetail&id=${listing.id}" class="block w-full text-center border border-gray-300 text-gray-600 text-xs font-medium py-2 rounded-md hover:bg-gray-50 transition-colors">
              <i class="fa-solid fa-eye mr-1"></i> View Details
            </a>
          </div>
        ` : ''}
