  ...
  if (!empty($oldInput) && (isset($oldInput['name']) || isset($oldInput['confirm_password']))) {
    $activeTab = 'signup';
  }

  // Get flash messages once to distribute to active tab
  $flashMessages = getFlashMessages();

  // Helper to render flash messages inline
  function renderLocalFlashMessages($messages)
  {
    ...
    foreach ($messages as $flash) {
      ...
      $styles = [
        ...
        'warning' => ['bg' => 'bg-yellow-50', 'border' => 'border-yellow-500', 'text' => 'text-yellow-800', 'icon' => '⚠', 'iconBg' => 'bg-yellow-500'],
        'info' => ['bg' => 'bg-blue-50', 'border' => 'border-blue-500', 'text' => 'text-blue-800', 'icon' => 'ℹ', 'iconBg' => 'bg-blue-500']
      ];

      $style = $styles[$type] ?? $styles['info'];

      echo "<div class=\"{$style['bg']} border-l-4 {$style['border']} p-4 rounded-xl flex items-start shadow-sm animate-fade-in\">";
      echo "<div class=\"flex-shrink-0 w-6 h-6 {$style['iconBg']} rounded-full flex items-center justify-center text-white text-xs font-bold mt-0.5 mr-3\">{$style['icon']}</div>";
      echo "<div class=\"flex-1 {$style['text']} text-sm font-medium\">{$msg}</div>";
      echo "</div>";
    }
    echo '</div>';
  }
      ...
      <div class="flex w-full max-w-6xl mx-auto bg-white rounded-3xl shadow-2xl overflow-hidden min-h-[600px]">
        ...
        <div class="w-full lg:w-3/5 p-8 lg:p-12 flex items-center justify-center">
          <div class="w-full max-w-md">
            ...
            <!-- Display Flash Messages (Removed global display) -->
            <?php // displayFlashMessages(); moved to inside tabs 
            ?>
            ...
            <?php if ($activeTab == 'login'): ?>
              <div class="auth-tab-content" id="login-content">
                <form class="space-y-6" action="<?= url('auth_login') ?>" method="post" id="loginForm" novalidate>
                  <?php csrfTokenField(); ?>
                  ...
                  <div class="flex items-center justify-between">
                    <a href="<?= url('forgotPassword') ?>" class="text-sm font-medium text-blue-600 hover:text-blue-500 transition-colors duration-200">Forgot password?</a>
                  </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                  <a href="<?= url('auth_google_callback') ?>"
                  ...
                  <a href="<?= url('auth_facebook_callback') ?>"
                    class="flex items-center justify-center px-4 py-3 border border-gray-300 rounded-xl hover:bg-gray-50 transition-all duration-200 group">
            ...
            <?php if ($activeTab == 'signup'): ?>
              <div class="auth-tab-content" id="signup-content">
                <form class="space-y-6" action="<?= url('auth_submit') ?>" method="post" id="signupForm" novalidate>
                  <?php csrfTokenField(); ?>
                  <?php renderLocalFlashMessages($flashMessages); ?>
