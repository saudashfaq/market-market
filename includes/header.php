...
<head>
  ...
  <script src="<?= url('js/popup.js') ?>?v=<?= time() ?>"></script>
  <?php if (is_logged_in()): ?>
    <script>
      (function() {
        // --------------------------------------------------------------------------
        // 🌍 Dynamic Environment Configuration (Server vs Local)
        // Generated by PHP to ensure 100% accuracy based on config.php
        // --------------------------------------------------------------------------
        ...
        <?php
        // Determine the correct API base and endpoint style based on environment
        if (defined('IS_PUBLIC_ROOT') && IS_PUBLIC_ROOT) {
          // 🚀 PRODUCTION / SERVER (Nginx with root /public)
          // URLs are routed via index.php, so we use CLEAN URLs (no .php, no /api/ prefix in URL)
          // Example: http://IP/polling_integration
          $apiBase = rtrim(BASE, '/');
          $pollingUrl = $apiBase . '/polling_integration';
          $notifUrl = $apiBase . '/notifications_api';
        } else {
          // 💻 LOCAL / DEVELOPMENT (XAMPP)
          // URLs usually access files directly. API folder is sibling to public.
          // We need to strip 'public' from BASE to get to project root, then add 'api'
          $projectRoot = str_replace('/public', '', rtrim(BASE, '/'));
          $apiBase = $projectRoot . '/api';
          // Direct file access requires .php extension
          $pollingUrl = $apiBase . '/polling_integration.php';
          $notifUrl = $apiBase . '/notifications_api.php';
        }
        ?>

        window.API_BASE_PATH = "<?= $apiBase ?>";
        window.POLLING_URL = "<?= $pollingUrl ?>";
        window.NOTIFICATIONS_URL = "<?= $notifUrl ?>";

        console.log('🔧 Configuration Loaded:');
        console.log('  - BASE:', window.BASE);
        console.log('  - API Base:', window.API_BASE_PATH);
        console.log('  - Polling Endpoint:', window.POLLING_URL);
        console.log('  - Notif Endpoint:', window.NOTIFICATIONS_URL);
      })();
    </script>
    <script src="<?= url('js/notifications.js') ?>?v=<?= time() ?>"></script>
  <?php endif; ?>
  <script src="<?= url('js/logout-confirmation.js') ?>?v=<?= time() ?>" defer></script>
  <!-- Standardized Polling System -->
  <script>
    window.currentUserId = <?= is_logged_in() ? current_user()['id'] : 'null' ?>;
    window.currentUserRole = "<?= is_logged_in() ? (current_user()['role'] ?? 'user') : 'guest' ?>";
  </script>
  <script src="<?= url('js/path-detector.js') ?>?v=<?= time() ?>"></script>
  <script src="<?= url('js/polling.js') ?>?v=<?= time() ?>"></script>
  <script src="<?= url('js/polling-init.js') ?>?v=<?= time() ?>"></script>
  <style>
...
<body>
  ...
  <header class="text-gray-600 body-font border-b sticky top-0 z-50 bg-white shadow-sm">
    <div class="navbar-container container mx-auto flex flex-wrap p-3 sm:p-4 flex-row items-center justify-between">
      ...
      <div class="flex items-center space-x-3">
        <a href="<?= url('public/index.php?p=home') ?>" class="flex title-font font-medium items-center text-gray-900 flex-shrink-0">
        ...
        <?php
        $currentPath = $_GET['p'] ?? '';
        if ((defined('SHOW_SIDEBAR_TOGGLE') && SHOW_SIDEBAR_TOGGLE) || ($currentPath === 'dashboard' && is_logged_in())):
        ?>
          <button
      ...
      <nav class="navbar-nav hidden lg:flex items-center space-x-4 xl:space-x-6 text-sm xl:text-base text-gray-700">
        <?php if (is_logged_in()): ?>
          <a href="<?= url('public/index.php?p=listing') ?>" class="hover:text-gray-900 whitespace-nowrap">Browse Listings</a>
          <a href="<?= url('public/index.php?p=sellingOption') ?>" class="hover:text-gray-900 whitespace-nowrap">Sell Your Asset</a>
          <a href="<?= url('public/index.php?p=wishlist') ?>" class="hover:text-gray-900 whitespace-nowrap">Saved</a>
          <?php
          $user = current_user();
          ...
          ?>
          <a href="<?= url('public/index.php?p=dashboard&page=' . $dashboardPage) ?>" class="hover:text-gray-900 whitespace-nowrap">Dashboard</a>
        <?php endif; ?>
      </nav>
      ...
      <div class="hidden lg:flex items-center space-x-3 xl:space-x-5 flex-shrink-0">
        ...
        <?php if (is_logged_in()): ?>
          ...
          <a
            href="<?php
                  if ($role === 'superadmin') {
                    echo url('public/index.php?p=dashboard&page=superAdminDashboard');
                  } elseif ($role === 'admin') {
                    echo url('public/index.php?p=dashboard&page=adminDashboard');
                  } else {
                    echo url('public/index.php?p=dashboard&page=userDashboard');
                  }
                  ?>"
        ...
        <?php else: ?>
          <a href="<?= url('public/index.php?p=login') ?>" class="flex items-center gap-1 text-gray-700 hover:text-gray-900 text-sm xl:text-base whitespace-nowrap">
        ...
        <?php if (is_logged_in()): ?>
          <a href="<?= url('public/index.php?p=sellingOption') ?>"
        ...
        <?php else: ?>
          <a href="<?= url('public/index.php?p=login') ?>"
            class="px-3 xl:px-4 py-2 rounded-lg text-white font-medium bg-gradient-to-r from-indigo-500 to-purple-500 hover:opacity-90 transition text-sm xl:text-base whitespace-nowrap">
            Start Selling
          </a>
        <?php endif; ?>
    ...
    <div id="mobile-menu" class="hidden lg:hidden bg-white shadow-lg animate-slideDown">
      <div class="container mx-auto px-4 py-3">
        <div class="flex flex-col space-y-1">
          <?php if (is_logged_in()): ?>
            <a href="<?= url('public/index.php?p=listing') ?>" class="py-3 px-2 text-gray-700 hover:bg-gray-100 rounded-lg transition flex items-center">
              <i class="fa-solid fa-compass mr-3 text-gray-500"></i> Browse Listings
            </a>
            <a href="<?= url('public/index.php?p=wishlist') ?>" class="py-3 px-2 text-gray-700 hover:bg-gray-100 rounded-lg transition flex items-center">
          ...
          <?php if (is_logged_in()): ?>
            ...
            switch ($role) {
              case 'superadmin':
                $dashboardLink = url('public/index.php?p=dashboard&page=superadminDashboard');
                break;
              case 'admin':
                $dashboardLink = url('public/index.php?p=dashboard&page=adminDashboard');
                break;
              default:
                $dashboardLink = url('public/index.php?p=dashboard&page=userDashboard');
                break;
            }
            ?>
          ...
          <?php else: ?>
            <a href="<?= url('public/index.php?p=login') ?>" class="py-3 px-2 text-gray-700 hover:bg-gray-100 rounded-lg transition flex items-center">
  ...
  <script>
    ...
    function loadNotificationsDirectly(listId) {
      fetch('<?= BASE ?>api/notifications_api.php?action=list&limit=10')
